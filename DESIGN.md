...
| **カテゴリ管理** | カテゴリ作成・編集・削除・並び替え、エントリへのカテゴリ割当、ホーム画面でカテゴリ別アコーディオン表示 |
...
| **詳細画面操作性向上** | 詳細画面下部に固定アクション（QR表示/URL起動/情報編集）を常時表示し、中央左右のフローティングボタンでも前後移動が可能 |
| **QR生成の高度設定** | 設定画面のアコーディオンで誤り訂正レベル・余白・ギャップレス描画を変更し、QR描画に反映 |
| **QR 表示サイズ調整** | QR コードの表示サイズを大・中・小プリセットまたはスライダーで自由に変更 |
...
| `/export` | `ExportPage` | - | エクスポート |
| `/import` | `ImportPage` | - | インポート |
...
| **EditPage** | `SwitchListTile` でお気に入りの ON/OFF |
...
| iCloud / Google Drive 連携 | クラウドバックアップ |
